...
<head>
	...
	<style>
		...
		* { box-sizing: border-box; }
		...
		.stage { position: relative; height: 280px; border-radius: 14px; overflow: hidden; background: #070b12; border: 1px solid var(--line); }
		.stage canvas { display: block; width: 100%; height: 100%; }
		.labels { position: absolute; left: 8px; right: 8px; top: 10px; display: flex; flex-direction: column; align-items: center; gap: 3px; pointer-events: none; z-index: 1; }
		.plate { font-family: "Minecraft", "Courier New", monospace; font-size: 14px; line-height: 1.2; text-shadow: 1px 1px #000; background: #000c; border: 1px solid var(--line); border-radius: 6px; padding: 3px 8px; color: #fff; max-width: 100%; }
		.plate.tag { font-size: 13px; }
		...
		.nocape { color: var(--muted); font-size: 12px; margin: 8px 0 0; }
...
<body>
		...
		<div class="drawer">
			...
			<div class="preview">
				<div class="stage" id="d-stage">
					...
					<div class="labels">
						<div class="plate tag" id="d-htag" hidden></div>
						<div class="plate name" id="d-nname"></div>
					</div>
	...
	<script>
		...
		function loadModel(player) {
			...
			viewer.setSize(Math.max(220, stage.clientWidth || 360), 280);
			...
			if (player.cape) {
				viewer.loadCape("/capes/" + player.uuid + ".png?h=" + encodeURIComponent(player.hash || Date.now())).catch(function () {
		...
		window.addEventListener("resize", function () {
			...
			skinViewer.setSize(Math.max(220, document.getElementById("d-stage").clientWidth), 280);
		});
