...
2. Drop [Fabric API](https://modrinth.com/mod/fabric-api/versions?g=26.1.2) into `mods`.
		3. Build this project (`./gradlew build`) and put `build/libs/voidmark-1.1.82.jar` in `mods`.
...
- The toolbar gear opens **Theme**: **Accent**, **Pane** color, and **Opacity** (the menu pane plus every HUD that uses that pane). Animation toggle is there too.
...
Markers only run in Skyblock by default. Enable **Force enable** on the Nodes cog to test in singleplayer with magenta terracotta. Status (Hypixel / Skyblock / The End, FPS, ping) is on the same page.
...
| Player | Nametags, nick hider, custom cape |
