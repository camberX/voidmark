...
3. Build this project (`./gradlew build`) and put `build/libs/voidmark-1.1.29.jar` in `mods`.
...
- **Inventory HUD** (Display, or the bell) draws your armor, storage, and hotbar on-screen. It reads the live inventory every frame — pickups, counts, durability, and cooldowns update without opening the inventory. The panel uses the same chrome as the click GUI. Hide it with F1; it also hides while a chest or the vanilla inventory is open.
...
| Display | Node HUD, watermark, inventory HUD, boxes, tracers, marker color |
