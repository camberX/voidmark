...
3. Build this project (`./gradlew build`) and put `build/libs/voidmark-1.1.104.jar` in `mods`.
...
- **Nametags** (ESP tab cog) are Voidmark-styled name plates in the default Minecraft font that keep drawing past vanilla’s 64-block cutoff (range 64–256m). They scale with distance and a Size slider (50–200%), plus Opacity. Optional distance text (not on your own tag) and through-walls. **Own nametag** (off in first person either way) shows your plate in F5; **VOIDMARK Dev** only draws when that is on. UUID v2 entities (Hypixel NPCs) are skipped; only UUID v4 players get a plate. The Dev line uses the menu font and sits in the same plate as the name. If custom nametags are off, it uses vanilla nametag chrome stacked above the name with a gap so they do not overlap. Vanilla tags are hidden while custom nametags are on so they do not stack. Your nick replaces your own name in F5.
