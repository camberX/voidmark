...
3. Build this project (`./gradlew build`) and put `build/libs/voidmark-1.1.152.jar` in `mods`.
...
Click one or more types in the scrollable list of every living vanilla type plus players. Click a selected row again to deselect it. Matching entities are drawn into the outline buffer as a silhouette, then a custom post shader blurs that mask and keeps only the outside so you get a clean rim plus an outward gradient. `/vm esp <text>` does the same for nametags that contain that word. After you see one named copy in this world, Voidmark remembers that mob's type and armor and glows the rest at render distance, even before their hologram plate draws. Changing worlds forgets that look so you have to see one named copy again. Entities that already glow in vanilla are left on Minecraft’s sobel outline. **Block outline** applies the same glow to the block under your crosshair; its color and opacity are on the Block outline cog. Through-walls, size, opacity, and color for mobs are on the Mob glow cog.
