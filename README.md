...
3. Build this project (`./gradlew build`) and put `build/libs/voidmark-1.1.140.jar` in `mods`.
...
- `/vm edit` or `/voidmark edit` opens the item id window. It shows the item you are holding, with its `minecraft:` id or Skyblock `sb:` id in a text box and a large preview above it. Typing another id reskins that item on your client: hand, hotbar, inventory, and worn armor all show the new look (armor uses the copied piece's 3D model, including player-head helmets). The server still has the real item. Vanilla ids look like `minecraft:diamond_sword`. Skyblock ids look like `sb:HYPERION`, or just the name (`Hyperion`). Tab or click a suggestion to fill it. Type the original id again to clear the reskin. Typing a Skyblock name also copies that item's Hypixel name and lore (color codes included) onto the item you are holding. **Maxed** (on Skyblock items) fills in recombobulated rarity, dungeon stars and master stars, gemstone slots, hot-potato stats, and typical max enchants — only the parts that item already supports.
...
- **ESP** (ESP tab) highlights every loaded entity of the types you pick, including other players. Scroll the full vanilla list (or type in the list search) and click rows to select them; click a selected row again to drop it. Matching entities get a **silhouette outline** with a shader gradient that fades outward from the model — not boxes, and not Minecraft’s sobel glow. You are never outlined. **Chams** paints the model itself: **Fill** is a full unlit solid, **Default** is a solid color that still takes lighting and shadows, **Tint** washes the real skin with a color. Through-walls, opacity, and color sit on the Chams cog. The left column has a rotating **3D preview** of you with the current glow and chams applied; drag to spin it. **Block outline** (on by default) puts that same glow on the block you are looking at, around the vanilla selection wire. Color and opacity sit on the cog next to Block outline. **Nametags** sit on the same Glow card.
...
- **Nametags** (ESP tab cog) default to Voidmark-styled name plates in the Minecraft font that keep drawing past vanilla’s 64-block cutoff (range 64–256m). **Style** on that cog switches to **Vanilla** so Minecraft’s own tags show instead. Custom plates scale with distance and a Size slider (50–200%), plus Opacity. Optional distance text (not on your own tag) and through-walls. **Own nametag** is its own switch on ESP (off in first person either way) and shows your plate in F5; **VOIDMARK Dev** only draws when that is on. UUID v2 entities (Hypixel NPCs) are skipped; only UUID v4 players get a plate. The Dev line uses the menu font and sits in the same plate as the name. If custom nametags are off, it uses vanilla nametag chrome stacked above the name with a gap so they do not overlap. Vanilla tags are hidden while Voidmark plates are on so they do not stack. Your nick replaces your own name in F5.
...
Click one or more types in the scrollable list of every living vanilla type plus players. Click a selected row again to deselect it. Matching entities are drawn into the outline buffer as a silhouette, then a custom post shader blurs that mask and keeps only the outside so you get a clean rim plus an outward gradient. **Chams** (own cog) recolors the model: Fill is unlit solid, Default keeps shadows on a solid color, Tint multiplies the skin. Through-walls, opacity, and color are on that cog. The Preview card under Glow shows your player with the live glow and chams settings. **Block outline** applies the same glow to the block under your crosshair; its color and opacity are on the Block outline cog. Through-walls, size, opacity, and color for mobs are on the Mob glow cog.
...
| ESP | Mob list, glow, chams, 3D preview, block outline, nametags |
