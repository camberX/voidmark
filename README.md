...
3. Build this project (`./gradlew build`) and put `build/libs/voidmark-1.1.135.jar` in `mods`.
...
- `/vm edit` or `/voidmark edit` opens the item id window. It shows the item you are holding, with its `minecraft:` id or Skyblock `sb:` id in a text box and a large preview above it. Typing another id reskins that item on your client: hand, hotbar, and inventory all show the new look. The server still has the real item. Vanilla ids look like `minecraft:diamond_sword`. Skyblock ids look like `sb:HYPERION`, or just the name (`Hyperion`). Tab or click a suggestion to fill it. Type the original id again to clear the reskin. Typing a Skyblock name also copies that item's Hypixel name and lore (color codes included) onto the item you are holding.
