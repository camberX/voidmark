...
**Hitsound** plays a short clip as soon as **you** land a hit — left-click on a mob or hologram, or **your** arrow overlapping a mob. Other players' hits do not ding. Hypixel often syncs mob health as 0, so Voidmark does not wait for the client to think the mob is alive. Real players (UUID v4) are skipped; Hypixel NPCs still count. **Melee** and **Arrows** are separate switches; volume and pitch are on the cog. Turning Enable on plays a preview. The delayed vanilla arrow ping is muted if Voidmark already predicted that shot.
