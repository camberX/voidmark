...
3. Build this project (`./gradlew build`) and put `build/libs/voidmark-1.1.103.jar` in `mods`.
...
- **Nametags** (ESP tab cog) are Voidmark-styled name plates in the default Minecraft font that keep drawing past vanilla’s 64-block cutoff (range 64–256m). They scale with distance and a Size slider (50–200%), plus Opacity. Optional distance text (not on your own tag) and through-walls. UUID v2 entities (Hypixel NPCs) are skipped; only UUID v4 players get a plate. The **VOIDMARK Dev** badge uses the menu font and sits in the same plate as the name, only as wide as needed. If custom nametags are off, it still draws above the vanilla name in that same square background, menu font kept. Vanilla tags are hidden while custom nametags are on so they do not stack. Your nick replaces your own name in F5.
