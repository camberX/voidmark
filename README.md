...
3. Build this project (`./gradlew build`) and put `build/libs/voidmark-1.1.65.jar` in `mods`.
...
- `/vm rawmats sb:HYPERION` (or `/voidmark rawmats`, or `/vm rawmats` while holding the item) expands that Skyblock craft to raw materials and shows a HUD with item icons, have/need counts, and a live progress bar per ingredient. It counts your inventory, armor, and any Ender Chest / backpack pages you open this session, including upgraded counterparts (starred dungeon items, higher Crimson tiers) and crafted intermediates (an Enchanted Diamond counts as 160 Diamond). `/vm rawmats clear` hides the tracker. Display tab → Raw mats HUD, or the bell overlay list.
